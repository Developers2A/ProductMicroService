...
{
    public class SuspendOrderCommandHandler : IRequestHandler<SuspendOrderCommand, BaseResponse<List<PostSuspendOrderResponse>>>
    {
        private readonly IConfiguration _configuration;
        private readonly CourierConfig _gateway;
        private readonly IMediator _mediator;

        public SuspendOrderCommandHandler(IConfiguration configuration, IMediator mediator)
        {
        ...
        public async Task<BaseResponse<List<PostSuspendOrderResponse>>> Handle(SuspendOrderCommand request, CancellationToken cancellationToken)
        {
            BaseResponse<List<PostSuspendOrderResponse>> result = new();
            try
            {
                ...
                try
                {
                    var resModel = JsonConvert.DeserializeObject<PostResponse<List<PostSuspendOrderResponse>>>(res);
        ...
        private async Task<HttpResponseMessage> SetHttpRequest(string token, SuspendOrderCommand request)
        {
            ...
            var pUrl = new Uri($"{_gateway.BaseUrl}Parcel/Suspend");
