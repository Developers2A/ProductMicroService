...
{
    public class GetChaparStatesQueryHandler : IRequestHandler<GetChaparStatesQuery, BaseResponse<List<ChaparState>>>
    {
        private readonly IConfiguration _configuration;
        private readonly CourierConfig _gateway;

        public GetChaparStatesQueryHandler(IConfiguration configuration)
        {
            _configuration = configuration;
            _gateway = _configuration.GetSection(nameof(CourierSetting)).Get<CourierSetting>().Chapar;
        }

        public async Task<BaseResponse<List<ChaparState>>> Handle(GetChaparStatesQuery request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var resModel = JsonConvert.DeserializeObject<ChaparGetStateResponse>(res);
                if (resModel!.result)
                {
        ...
        private async Task<HttpResponseMessage> SetHttpRequest(GetChaparStatesQuery request)
        {
