...
{
    public class GetPishroPostCitiesQueryHandler : IRequestHandler<GetPishroPostCitiesQuery, BaseResponse<List<ChaparCity>>>
    {
        private readonly IConfiguration _configuration;
        private readonly CourierConfig _gateway;

        public GetPishroPostCitiesQueryHandler(IConfiguration configuration)
        {
            _configuration = configuration;
            _gateway = _configuration.GetSection(nameof(CourierSetting)).Get<CourierSetting>().Chapar;
        }

        public async Task<BaseResponse<List<ChaparCity>>> Handle(GetPishroPostCitiesQuery request, CancellationToken cancellationToken)
        {
            try
            {
        ...
        private async Task<HttpResponseMessage> SetHttpRequest(GetPishroPostCitiesQuery request)
        {
