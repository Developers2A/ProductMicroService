...
{
    public class GetChaparProvincesQueryHandler : IRequestHandler<GetChaparProvincesQuery, BaseResponse<List<ChaparState>>>
    {
        private readonly IConfiguration _configuration;
        private readonly CourierConfig _gateway;

        public GetChaparProvincesQueryHandler(IConfiguration configuration)
        {
            _configuration = configuration;
            _gateway = _configuration.GetSection(nameof(CourierSetting)).Get<CourierSetting>().Chapar;
        }

        public async Task<BaseResponse<List<ChaparState>>> Handle(GetChaparProvincesQuery request, CancellationToken cancellationToken)
        {
            try
            {
        ...
        private async Task<HttpResponseMessage> SetHttpRequest(GetChaparProvincesQuery request)
        {
