...
{
    public class GetByUserAndValueAddedContractValueAddedQueryHandler : IRequestHandler<GetByUserAndValueAddedContractValueAddedQuery, ValueAddedPriceDto>
    {
        private readonly IReadRepository<ContractValueAdded> _readRepository;

        public GetByUserAndValueAddedContractValueAddedQueryHandler(IReadRepository<ContractValueAdded> readRepository)
        {
            _readRepository = readRepository;
        }
        public async Task<ValueAddedPriceDto> Handle(GetByUserAndValueAddedContractValueAddedQuery request, CancellationToken cancellationToken)
        {
            var ValueAddedPrice = await _readRepository.TableNoTracking
             .Include(c => c.ContractInfo).Include(x => x.ValueAddedType).Where(c => c.ContractInfo.IsActive == true && c.ContractInfo.UserId == null && c.ContractInfo.CityId == 0 && c.ContractInfo.ProvinceId == 0 && c.ContractInfo.StartDate <= DateTime.Now && c.ValueAddedTypeId == request.ValueAddedId)
            ...
            var itemCus = await _readRepository.Table
               .Include(c => c.ContractInfo).Where(c => c.ContractInfo.IsActive == true && c.ContractInfo.UserId == request.UserId && c.ContractInfo.ProvinceId == 0 && c.ContractInfo.StartDate <= DateTime.Now && c.ValueAddedTypeId == request.ValueAddedId)
                .Select(c => new ValueAddedPriceDto
                {
                    ContractId = c.ContractInfoId,
                    ContractValueAddedId = c.Id,
            ...
            var itemCity = await _readRepository.Table
              .Include(c => c.ContractInfo).Where(c => c.ContractInfo.IsActive == true && c.ContractInfo.CityId == request.CityId && c.ContractInfo.UserId == null && c.ContractInfo.StartDate <= DateTime.Now && c.ValueAddedTypeId == request.ValueAddedId)
             .Select(c => new ValueAddedPriceDto
             {
                 ContractId = c.ContractInfoId,
                 ContractValueAddedId = c.Id,
            ...
            var itemProvince = await _readRepository.Table
             .Include(c => c.ContractInfo).Where(c => c.ContractInfo.IsActive == true && c.ContractInfo.ProvinceId == request.ProvinceId && c.ContractInfo.CityId == 0 && c.ContractInfo.UserId == null && c.ContractInfo.StartDate <= DateTime.Now && c.ValueAddedTypeId == request.ValueAddedId)
              .Select(c => new ValueAddedPriceDto
              {
                  ContractId = c.ContractInfoId,
                  ContractValueAddedId = c.Id,
