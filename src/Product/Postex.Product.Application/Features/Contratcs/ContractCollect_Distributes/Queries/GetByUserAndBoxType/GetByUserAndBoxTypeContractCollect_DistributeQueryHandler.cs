...
namespace Postex.Product.Application.Features.Contratcs.ContractCollect_Distributes.Queries.GetByUserAndBoxType
{
    public class GetByUserAndBoxTypeContractCollect_DistributeQueryHandler : IRequestHandler<GetByUserAndBoxTypeContractCollect_DistributeQuery, CollectionDistributionPriceDto>
    {
        private readonly IReadRepository<ContractCollectionDistribution> _readRepository;

        public GetByUserAndBoxTypeContractCollect_DistributeQueryHandler(IReadRepository<ContractCollectionDistribution> readRepository)
        {
            _readRepository = readRepository;
        }

        public async Task<CollectionDistributionPriceDto> Handle(GetByUserAndBoxTypeContractCollect_DistributeQuery request, CancellationToken cancellationToken)
        {
            var collect_DistributeDefualt = await _readRepository.Table
              .Include(c => c.ContractInfo).Where(c => c.ContractInfo.IsActive == true && c.CourierServiceId == request.CourierServiceId && c.ContractInfo.UserId == null && ((c.ContractInfo.ProvinceId == request.ProvinceId && c.ContractInfo.CityId == 0) || (c.ContractInfo.CityId == request.CityId && c.ContractInfo.ProvinceId == request.ProvinceId)) && c.ContractInfo.StartDate <= DateTime.Now && c.ContractInfo.EndDate >= DateTime.Now && c.BoxTypeId == request.BoxTypeId)
            ...
            var CusCity = await _readRepository.Table
               .Include(c => c.ContractInfo).Where(c => c.ContractInfo.IsActive == true && c.CourierServiceId == request.CourierServiceId && c.ContractInfo.UserId == request.UserId && c.ProvinceId == request.ProvinceId && c.CityId == request.CityId
            ...
            var CusProvince = await _readRepository.Table
                .Include(c => c.ContractInfo).Where(c => c.ContractInfo.IsActive == true && c.CourierServiceId == request.CourierServiceId && c.ContractInfo.UserId == request.UserId && c.ProvinceId == request.ProvinceId && c.CityId == 0
                                              && c.ContractInfo.StartDate <= DateTime.Now && c.ContractInfo.EndDate >= DateTime.Now && c.BoxTypeId == request.BoxTypeId)
                .Select(c => new CollectionDistributionPriceDto
                {
                    BoxTypeId = c.BoxTypeId,
            ...
            var Cus = await _readRepository.Table
               .Include(c => c.ContractInfo).Where(c => c.ContractInfo.IsActive == true && c.CourierServiceId == request.CourierServiceId && c.ContractInfo.UserId == request.UserId && c.ProvinceId == 0 && c.CityId == 0
                                             && c.ContractInfo.StartDate <= DateTime.Now && c.ContractInfo.EndDate >= DateTime.Now && c.BoxTypeId == request.BoxTypeId)
               .Select(c => new CollectionDistributionPriceDto
               {
                   BoxTypeId = c.BoxTypeId,
